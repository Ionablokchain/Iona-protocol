...
//! Voting power is weighted by the validator's stake (via `StakeLedger`).
//!
//! # Example
//!
//! ```
...
impl GovernanceState {
    /// Create a new governance state with default parameters.
    pub fn new() -> Self {
        Self {
            next_id: 0,
    ...
    /// Returns `(yes_power, no_power, total_power, quorum_reached, passes)`.
    pub fn tally_weighted(
        &self,
        proposal_id: u64,
        stakes: &BTreeMap<String, u128>,
    ) -> (u128, u128, u128, bool, bool) {
        ...
        let quorum_needed = total_power * self.quorum_bps as u128 / 10_000;
        let threshold_needed = total_power * self.threshold_bps as u128 / 10_000;
        let quorum_reached = yes_power + no_power >= quorum_needed;
        let passes = quorum_reached && yes_power >= threshold_needed;
        (yes_power, no_power, total_power, quorum_reached, passes)
    }
    ...
    /// Update governance parameters (e.g., after a successful ParamChange proposal).
...
mod tests {
    ...
    fn test_submit_insufficient_deposit() {
        ...
        let err = gov.submit(kind, DEFAULT_MIN_DEPOSIT - 1, 10, 5).unwrap_err();
        assert!(matches!(err, GovernanceError::InsufficientDeposit { required: DEFAULT_MIN_DEPOSIT, provided: _ }));
    }
    ...
    fn test_tally_weighted() {
        ...
        let (yes, no, total, quorum, passes) = gov.tally_weighted(id, &stakes);
        assert_eq!(yes, 1_000);
        assert_eq!(no, 500);
        assert_eq!(total, 3_500);
        assert!(quorum); // 1500 >= 1167 (33.4% of 3500)
        assert!(passes); // 1000 >= 1750? No, 1000 < 1750 (50% of 3500) => should be false
        // Actually yes_power = 1000, threshold_needed = 1750, so passes = false.
        assert!(!passes);
    }
    ...
    fn test_prune_expired() -> GovernanceResult<()> {
        let mut gov = GovernanceState::new();
        gov.submit(ProposalKind::ParamChange { key: "p1".into(), value: "v1".into() }, DEFAULT_MIN_DEPOSIT, 10, 5)?;
        gov.submit(ProposalKind::ParamChange { key: "p2".into(), value: "v2".into() }, DEFAULT_MIN_DEPOSIT, 20, 10)?;
