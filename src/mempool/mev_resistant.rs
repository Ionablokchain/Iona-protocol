...
//! │ (MevConfig) │ (MevError)   │ (MevMetrics)  │ (Commit, Reveal, Envelope)│
//! ├─────────────┼──────────────┼───────────────┼──────────────────────────┤
//! │   Crypto    │   Ordering   │    Mempool    │        Manager           │
//! │ (hash, enc) │ (fair shuffle)│ (MevMempool) │ (MevManager)             │
//! └─────────────────────────────────────────────────────────────────────────┘
//! ```
//!
//! # Example
//!
//! ```rust,ignore
//! use iona::mempool::mev::{MevManager, MevConfigBuilder};
//!
//! let config = MevConfigBuilder::new()
//!     .commit_ttl_blocks(30)
//!     .enable_threshold_encryption(true)
//!     .build()?;
//! let mut manager = MevManager::new(config);
//! manager.submit_tx(tx)?;
//! let txs = manager.drain_fair(100);
//! ```
...
mod config {
    //! Configuration for the MEV mempool.
    use serde::{Deserialize, Serialize};
...
mod error {
    //! Error types for the MEV mempool.
    use thiserror::Error;
...
mod types {
    //! Core types for the MEV mempool.
    use super::{crypto::Hash32, Height};
    use crate::types::Tx;
    use serde::{Deserialize, Serialize};
...
mod crypto {
    //! Cryptographic helpers for commit-reveal and threshold encryption.
    use super::{error::MevResult, error::MevError, types::Hash32};
    use crate::types::{hash_bytes, Tx};
    ...
    pub fn generate_salt(tx: &Tx, min_length: usize) -> Vec<u8> {
    ...
    pub fn encrypt_tx_envelope(
        ...
        epoch_secret: &[u8; 32],
        epoch: u64,
    ) -> MevResult<super::types::EncryptedEnvelope> {
        ...
        Ok(super::types::EncryptedEnvelope {
            ciphertext,
            nonce: nonce_bytes,
            epoch,
    ...
    pub fn decrypt_tx_envelope(
        envelope: &super::types::EncryptedEnvelope,
        epoch_secret: &[u8; 32],
    ) -> MevResult<Tx> {
        let cipher = Aes256Gcm::new_from_slice(epoch_secret)
...
mod ordering {
    //! Quantum Fair Ordering — deterministic shuffling with jitter bucketing.
    use super::{types::Hash32, types::TxCommit};
...
mod mempool {
    //! Core MEV‑resistant mempool implementation.
    use super::{
        config::MevConfig,
        error::{MevError, MevResult},
        metrics::MevMetrics,
        types::{TxCommit, TxReveal, EncryptedEnvelope},
        crypto::{compute_commit_hash, decrypt_tx_envelope, generate_salt},
        ordering::fair_order_shuffle,
    };
    ...
    impl MevMempool {
        ...
        pub fn decrypt_pending(&mut self, epoch_secret: &[u8; 32]) -> Vec<Tx> {
            ...
            while let Some(envelope) = self.encrypted_queue.pop_front() {
                match super::crypto::decrypt_tx_envelope(&envelope, epoch_secret) {
                    Ok(tx) => {
        ...
        pub fn metrics(&self) -> &MevMetrics {
            &self.metrics
        }
...
mod manager {
    //! Centralised manager for the MEV mempool.
    use super::{
        config::MevConfig,
        error::MevResult,
        mempool::MevMempool,
        metrics::MevMetrics,
        types::{Hash32, Height, Tx, TxCommit, TxReveal, EncryptedEnvelope},
    };
    use std::time::Duration;

    /// Manager for the MEV mempool.
    ...
    impl MevManager {
        pub fn new(config: MevConfig) -> MevResult<Self> {
            let mempool = MevMempool::new(config)?;
            Ok(Self { mempool })
        }

        pub fn with_defaults() -> MevResult<Self> {
        ...
        pub fn submit_commit(&mut self, commit: TxCommit) -> MevResult<()> {
            self.mempool.submit_commit(commit)
        }
        ...
        pub fn submit_encrypted(&mut self, envelope: EncryptedEnvelope) -> MevResult<()> {
            self.mempool.submit_encrypted(envelope)
        }
        ...
        pub fn decrypt_pending(&mut self, epoch_secret: &[u8; 32]) -> Vec<Tx> {
            self.mempool.decrypt_pending(epoch_secret)
        }

        pub fn drain_fair(&mut self, n: usize) -> Vec<Tx> {
            self.mempool.drain_fair(n)
        }

        pub fn drain_all_revealed(&mut self) -> Vec<Tx> {
            self.mempool.drain_all_revealed()
        }

        pub fn peek_revealed(&self, n: usize) -> Vec<&Tx> {
            self.mempool.peek_revealed(n)
        }

        pub fn advance_height(&mut self, height: Height, block_hash: &Hash32) {
            self.mempool.advance_height(height, block_hash)
        }

        pub fn clear_expired(&mut self) -> usize {
            self.mempool.clear_expired()
        }

        pub fn record_proposer(&mut self, height: Height, proposer: String) {
            self.mempool.record_proposer(height, proposer)
        }

        pub fn pending_commit_count(&self) -> usize {
            self.mempool.pending_commit_count()
        }

        pub fn revealed_count(&self) -> usize {
            self.mempool.revealed_count()
        }

        pub fn encrypted_count(&self) -> usize {
            self.mempool.encrypted_count()
        }
...
pub use config::{MevConfig, MevConfigBuilder};
pub use error::{MevError, MevResult};
pub use types::{TxCommit, TxReveal, CommitStatus, EncryptedEnvelope};
pub use metrics::MevMetrics;
pub use crypto::{compute_commit_hash, generate_salt, derive_epoch_secret, encrypt_tx_envelope, decrypt_tx_envelope};
pub use mempool::MevMempool;
pub use manager::MevManager;

// -----------------------------------------------------------------------------
// Legacy global API (backward compatibility)
// -----------------------------------------------------------------------------

/// Create a new MEV mempool with default configuration.
pub fn new_default() -> MevResult<MevMempool> {
    MevMempool::new(MevConfig::default())
}

/// Create a new MEV mempool with custom configuration.
pub fn new_with_config(config: MevConfig) -> MevResult<MevMempool> {
...
mod tests {
    ...
    fn test_fair_ordering_determinism() {
        ...
        ordering::fair_order_shuffle(&mut items1, 50, &seed);
        ordering::fair_order_shuffle(&mut items2, 50, &seed);
