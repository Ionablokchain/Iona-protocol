...
//! │ (InvariantCfg)│ (InvariantError)│ (ST-1–ST-11)│ (InvariantCheck, Rpt) │
//! ├─────────────┼──────────────┼───────────────┼──────────────────────────┤
//! │                       validator (StateInvariantValidator)              │
...
//!
//! ```rust,ignore
//! use iona::protocol::state_invariants::{StateInvariantValidator, InvariantConfig};
//! use iona::types::Block;
//! use iona::execution::KvState;
//!
//! let config = InvariantConfig::default();
//! let validator = StateInvariantValidator::new(config);
//! let report = validator.check_block(&block, 1, 1000, &[], &state);
...
pub mod config {
    //! Configuration for state invariant checks.
    use serde::{Deserialize, Serialize};
    ...
    pub struct InvariantConfig {
        ...
        /// Whether to fail on warning‑level checks (strict mode).
        pub strict: bool,
    ...
    impl InvariantConfig {
        pub fn validate(&self) -> Result<(), &'static str> {
            if self.supply_tolerance == 0 {
                return Err("supply_tolerance must be > 0");
            }
            Ok(())
        }
...
pub mod error {
    //! Error types for state invariant checks.
...
pub mod check {
    ...
    use crate::types::{tx_hash, Block, Height, Receipt, tx_root, receipts_root, Hash32};
    use crate::execution::KvState;
    use std::collections::BTreeMap;
    use tracing::debug;

    /// ST-1: Verify no account has a negative balance (u64 guard).
...
pub mod report {
    //! Reports for state invariant checks.
    use serde::{Deserialize, Serialize};
...
pub mod metrics {
    //! Metrics for state invariant checks.
    use std::sync::atomic::{AtomicU64, Ordering};
...
pub mod validator {
    //! Centralised validator for state invariants.
    use super::{
        config::InvariantConfig,
        error::StateInvariantResult,
        check::{
            check_balances_non_negative, check_nonces_monotonic, check_supply_conservation,
            check_state_root_determinism, check_height_monotonic, check_timestamp_monotonic,
            check_tx_uniqueness, check_gas_accounting, check_tx_root, check_receipts_root,
            check_state_root,
        },
    ...
    #[derive(Debug, Clone)]
    pub struct StateInvariantValidator {
        config: InvariantConfig,
        metrics: Arc<InvariantMetrics>,
    }

    impl StateInvariantValidator {
                ...
                ($id:expr, $name:expr, $enabled:expr, $expr:expr) => {
                    if $enabled {
                        ...
                        if !passed && self.config.strict {
                            warn!(check = $id, detail, "invariant failed (strict)");
                        } else if !passed {
                            debug!(check = $id, detail, "invariant failed (non‑strict)");
                        }
                    } else {
                        checks.push(InvariantCheck::new($id, $name, true, "disabled", 0));
                    }
                };
            }

            run_check!("ST-5", "Height monotonic", self.config.enable_st5, {
                check_height_monotonic(prev_height, block.header.height)
            });

            run_check!("ST-6", "Timestamp monotonic", self.config.enable_st6, {
                check_timestamp_monotonic(prev_timestamp, block.header.timestamp)
            });

            run_check!("ST-7", "Tx uniqueness", self.config.enable_st7, {
                check_tx_uniqueness(block)
            });

            run_check!("ST-8", "Gas accounting", self.config.enable_st8, {
                check_gas_accounting(block.header.gas_used, receipts)
            });

            run_check!("ST-9", "Tx root match", self.config.enable_st9, {
                check_tx_root(block)
            });

            run_check!("ST-10", "Receipts root match", self.config.enable_st10, {
                check_receipts_root(block)
            });

            run_check!("ST-11", "State root match", self.config.enable_st11, {
        ...
        pub fn check_state(
            ...
            run_check!("ST-1", "Balance non‑negative", self.config.enable_st1, {
                check_balances_non_negative(balances_after)
            });

            run_check!("ST-2", "Nonce monotonic", self.config.enable_st2, {
                check_nonces_monotonic(nonces_before, nonces_after)
            });

            run_check!("ST-3", "Supply conservation", self.config.enable_st3, {
            ...
            run_check!("ST-4", "State root determinism", self.config.enable_st4, {
                check_state_root_determinism(state).map(|_| ())
            });
...
pub use validator::{StateInvariantValidator, SupplyDelta};
...
pub fn check_block_invariants(
    ...
    let validator = StateInvariantValidator::default();
    let dummy_state = KvState::default();
    validator.check_block(block, prev_height, prev_timestamp, receipts, &dummy_state)
}
...
mod tests {
    ...
    fn test_validator_state() {
        ...
        let state = KvState::default();
        let validator = StateInvariantValidator::default();
        let report = validator.check_state(
            &before, &after,
    ...
    fn test_validator_all() {
        ...
        let state = KvState::default();
        let validator = StateInvariantValidator::default();
        let report = validator.check_all(
            &block, 1, 1000, &[], &state,
            &before, &after,
            &nonces_before, &nonces_after,
            0, 0,
    ...
    fn test_validator_metrics() {
        ...
        let state = KvState::default();
        let _ = validator.check_block(&block, 1, 1000, &[], &state);
    ...
    fn test_strict_mode() {
        ...
        config.strict = true;
        // We'll create a block with an invalid height to force a failure.
        let block = Block {
            header: BlockHeader {
                height: 1, // same as prev_height, will fail ST-5
                ..Default::default()
            },
            txs: vec![],
            receipts: vec![],
        ...
        let validator = StateInvariantValidator::new(config);
        let state = KvState::default();
        let report = validator.check_block(&block, 1, 0, &[], &state);
        assert!(!report.all_passed);
        // In strict mode, the failure should be logged as a warning (via tracing).
        // We can't easily assert on logs, but we can check the report.
        assert_eq!(report.failures().len(), 1);
        assert_eq!(report.failures()[0].id, "ST-5");
    }
}
