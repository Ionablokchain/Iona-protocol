//! Rolling upgrade and migration tests for IONA v28.2.
//!
//! Tests:
//!  U1  — Fresh data directory: all migrations apply from v0 to CURRENT
//!  U2  — Already-current directory: all migrations skipped
//!  U3  — Partial migration: migrations resume from the interrupted version
//!  U4  — Dry-run: no files modified, but result is accurate
//!  U5  — Compatibility report: disk > binary schema → incompatible
//!  U6  — Compatibility report: disk < binary schema → migrations needed
//!  U7  — Compatibility report: disk == binary schema → compatible, no migrations
//!  U8  — Migration ordering: registry versions are 0-based, contiguous, ordered
//!  U9  — Individual migration idempotency (M001..M005)
//! U10  — Rolling upgrade simulation: 5 nodes, upgrade one at a time

use iona::storage::CURRENT_SCHEMA_VERSION;
use iona::upgrade::{
    check_compat, dry_run_migrations, CompatReport, MigrationRegistry, MigrationResult,
};
use std::fs;
use tempfile::TempDir;

// -----------------------------------------------------------------------------
// Constants
// -----------------------------------------------------------------------------

/// Schema file name.
const SCHEMA_FILE: &str = "schema.json";

/// Expected files after all migrations.
const EXPECTED_FILES: &[&str] = &["receipts", "evidence.json", "snapshots", "audit.log"];

/// Number of nodes in rolling upgrade test.
const NUM_NODES_ROLLING: usize = 5;

/// Schema version for "too new" test.
const FUTURE_SCHEMA_VERSION: u32 = CURRENT_SCHEMA_VERSION + 10;

/// Old schema version for migration test.
const OLD_SCHEMA_VERSION: u32 = 2;

/// Number of nodes in multi‑node upgrade test.
const NUM_NODES: usize = 5;

/// First migration version (0).
const VERSION_0: u32 = 0;

/// Partial migration version (simulate v2 applied).
const PARTIAL_VERSION: u32 = 2;

// -----------------------------------------------------------------------------
// Helpers
// -----------------------------------------------------------------------------

/// Create an empty temporary data directory (no `schema.json`).
fn empty_data_dir() -> TempDir {
    TempDir::new().expect("failed to create tempdir")
}

/// Create a data directory with a specific schema version in `schema.json`.
fn data_dir_at_version(version: u32) -> TempDir {
    let dir = TempDir::new().unwrap();
    let schema = serde_json::json!({ "version": version });
    let path = dir.path().join(SCHEMA_FILE);
    fs::write(path, serde_json::to_string_pretty(&schema).unwrap()).unwrap();
    dir
}

/// Write a schema version file to a directory.
fn write_schema_version(dir: &TempDir, version: u32) {
    let schema = serde_json::json!({ "version": version });
    let path = dir.path().join(SCHEMA_FILE);
    fs::write(path, serde_json::to_string_pretty(&schema).unwrap()).unwrap();
}

/// Check that all expected files exist after a successful migration.
fn assert_expected_files_exist(dir: &TempDir) {
    for &file in EXPECTED_FILES {
        assert!(
            dir.path().join(file).exists(),
            "Expected file/directory '{}' does not exist after migration",
            file
        );
    }
}

/// Check that none of the expected files exist (for dry‑run tests).
fn assert_expected_files_do_not_exist(dir: &TempDir) {
    for &file in EXPECTED_FILES {
        assert!(
            !dir.path().join(file).exists(),
            "File/directory '{}' should not exist after dry‑run",
            file
        );
    }
}

// -----------------------------------------------------------------------------
// U1: Fresh directory – all migrations applied
// -----------------------------------------------------------------------------

#[test]
fn u1_fresh_dir_all_migrations_applied() {
    let dir = empty_data_dir();
    let registry = MigrationRegistry::new();
    let results = registry.run(dir.path(), VERSION_0, /* dry_run = */ false);

    assert!(!results.is_empty(), "should have at least one migration");
    for result in &results {
        assert!(
            result.is_ok(),
            "all migrations must succeed on fresh dir: {result}"
        );
    }
}

#[test]
fn u1_fresh_dir_files_created() {
    let dir = empty_data_dir();
    let registry = MigrationRegistry::new();
    registry.run(dir.path(), VERSION_0, false);
    assert_expected_files_exist(&dir);
}

// -----------------------------------------------------------------------------
// U2: Already current directory – all migrations skipped
// -----------------------------------------------------------------------------

#[test]
fn u2_already_current_all_skipped() {
    let dir = empty_data_dir();
    let registry = MigrationRegistry::new();

    // First, apply all migrations.
    registry.run(dir.path(), VERSION_0, false);

    // Run again from current version – all must be skipped.
    let results = registry.run(dir.path(), CURRENT_SCHEMA_VERSION, false);
    for result in &results {
        assert!(
            matches!(result, MigrationResult::Skipped { .. }),
            "already‑applied migration must be skipped, got {result}"
        );
    }
}

// -----------------------------------------------------------------------------
// U3: Partial migration – resume from interrupted version
// -----------------------------------------------------------------------------

#[test]
fn u3_partial_migration_resumes() {
    let dir = empty_data_dir();
    let registry = MigrationRegistry::new();

    // Simulate: only M001 and M002 have run (schema at v2).
    // Apply all to create the necessary state, but then we query from v2.
    // The first run creates all files, but we then re‑run from version 2.
    // In a real scenario, only some files would exist. Our test is a bit
    // simplified but still checks that no errors occur when starting at v2.
    let _ = registry.run(dir.path(), VERSION_0, false);
    let results = registry.run(dir.path(), PARTIAL_VERSION, false);

    let all_ok = results.iter().all(|r| r.is_ok());
    assert!(
        all_ok,
        "partial migration from version {} must complete without error",
        PARTIAL_VERSION
    );
}

// -----------------------------------------------------------------------------
// U4: Dry‑run – no files created
// -----------------------------------------------------------------------------

#[test]
fn u4_dry_run_from_v0_no_files_created() {
    let dir = empty_data_dir();
    let registry = MigrationRegistry::new();
    let results = registry.run(dir.path(), VERSION_0, /* dry_run = */ true);

    for result in &results {
        assert!(result.is_ok(), "dry‑run must not fail: {result}");
    }

    assert_expected_files_do_not_exist(&dir);
}

#[test]
fn u4_dry_run_migrations_fn_returns_true_on_success() {
    let dir = empty_data_dir();
    write_schema_version(&dir, VERSION_0);
    let result = dry_run_migrations(dir.path());
    assert!(
        result.is_ok(),
        "dry_run_migrations must return Ok: {result:?}"
    );
}

// -----------------------------------------------------------------------------
// U5: Compatibility – disk newer than binary → incompatible
// -----------------------------------------------------------------------------

#[test]
fn u5_disk_newer_than_binary_reports_incompatible() {
    let report = CompatReport {
        disk_schema_version: FUTURE_SCHEMA_VERSION,
        binary_schema_version: CURRENT_SCHEMA_VERSION,
        binary_protocol_version: 1,
        compatible: false,
        migrations_needed: false,
        pending_migrations: 0,
        issues: vec!["on-disk schema is newer than binary".into()],
    };
    assert!(
        !report.is_ok(),
        "disk newer than binary must be incompatible"
    );
    let s = format!("{report}");
    assert!(s.contains("NO"), "report must say NOT compatible");
}

// -----------------------------------------------------------------------------
// U6: Compatibility – migrations needed
// -----------------------------------------------------------------------------

#[test]
fn u6_old_disk_needs_migrations() {
    let report = CompatReport {
        disk_schema_version: OLD_SCHEMA_VERSION,
        binary_schema_version: CURRENT_SCHEMA_VERSION,
        binary_protocol_version: 1,
        compatible: true,
        migrations_needed: true,
        pending_migrations: (CURRENT_SCHEMA_VERSION - OLD_SCHEMA_VERSION) as usize,
        issues: vec![],
    };
    assert!(
        report.compatible,
        "binary can open old directory (with migration)"
    );
    assert!(report.migrations_needed, "migration is needed");
    let s = format!("{report}");
    assert!(s.contains("YES"), "report must say migrations needed");
}

// -----------------------------------------------------------------------------
// U7: Compatibility – already current
// -----------------------------------------------------------------------------

#[test]
fn u7_current_dir_no_migration_needed() {
    let report = CompatReport {
        disk_schema_version: CURRENT_SCHEMA_VERSION,
        binary_schema_version: CURRENT_SCHEMA_VERSION,
        binary_protocol_version: 1,
        compatible: true,
        migrations_needed: false,
        pending_migrations: 0,
        issues: vec![],
    };
    assert!(report.is_ok());
    assert!(!report.migrations_needed);
}

// -----------------------------------------------------------------------------
// U8: Registry ordering and descriptions
// -----------------------------------------------------------------------------

#[test]
fn u8_registry_versions_ordered_and_contiguous() {
    let registry = MigrationRegistry::new();
    let versions: Vec<u32> = registry
        .all()
        .iter()
        .map(|m| m.from_version())
        .collect();
    for (i, &v) in versions.iter().enumerate() {
        assert_eq!(
            v, i as u32,
            "migration at index {i} must have from_version={i}, got {v}"
        );
    }
}

#[test]
fn u8_registry_descriptions_are_non_empty() {
    let registry = MigrationRegistry::new();
    for migration in registry.all() {
        assert!(
            !migration.description().is_empty(),
            "migration v{} must have a non‑empty description",
            migration.from_version()
        );
    }
}

// -----------------------------------------------------------------------------
// U9: Each migration is idempotent
// -----------------------------------------------------------------------------

#[test]
fn u9_each_migration_is_idempotent() {
    let dir = empty_data_dir();
    let registry = MigrationRegistry::new();

    // First run – apply all.
    registry.run(dir.path(), VERSION_0, false);

    // Second run – must be idempotent (all results OK).
    let results = registry.run(dir.path(), VERSION_0, false);
    for result in &results {
        assert!(
            result.is_ok(),
            "second run must not fail (idempotency): {result}"
        );
    }
}

// -----------------------------------------------------------------------------
// U10: Rolling upgrade simulation – 5 nodes, upgrade one at a time
// -----------------------------------------------------------------------------

#[test]
fn u10_rolling_upgrade_5_nodes() {
    use iona::protocol::version::CURRENT_PROTOCOL_VERSION;

    let dirs: Vec<TempDir> = (0..NUM_NODES_ROLLING)
        .map(|_| empty_data_dir())
        .collect();
    let registry = MigrationRegistry::new();

    // Phase 1: all nodes start at v0 (no migrations yet).
    // Phase 2: upgrade nodes one by one.
    let mut upgraded = 0;
    for dir in &dirs {
        let results = registry.run(dir.path(), VERSION_0, false);
        for result in &results {
            assert!(result.is_ok(), "node upgrade must succeed: {result}");
        }
        upgraded += 1;
        // After each upgrade, the protocol version must remain unchanged.
        assert_eq!(
            CURRENT_PROTOCOL_VERSION, 1,
            "protocol version must remain 1 during rolling schema upgrade"
        );
    }
    assert_eq!(
        upgraded, NUM_NODES_ROLLING,
        "all {} nodes must upgrade",
        NUM_NODES_ROLLING
    );

    // Phase 3: all nodes upgraded – verify expected files exist.
    for dir in &dirs {
        assert!(
            dir.path().join("audit.log").exists(),
            "every upgraded node must have audit.log"
        );
        assert!(
            dir.path().join("evidence.json").exists(),
            "every upgraded node must have evidence.json"
        );
    }
}

// -----------------------------------------------------------------------------
// MigrationResult display tests
// -----------------------------------------------------------------------------

#[test]
fn migration_result_ok_display() {
    let result = MigrationResult::Ok {
        from_version: 0,
        to_version: 1,
        changes: vec!["added vm field".into(), "normalised balances".into()],
    };
    let s = format!("{result}");
    assert!(
        s.contains("v0 → v1"),
        "display must show version transition"
    );
    assert!(s.contains("added vm field"), "display must list changes");
}

#[test]
fn migration_result_failed_display() {
    let result = MigrationResult::Failed {
        from_version: 2,
        reason: "disk full".into(),
    };
    let s = format!("{result}");
    assert!(s.contains("FAILED"), "failed result must say FAILED");
    assert!(s.contains("disk full"), "failed result must include reason");
}
